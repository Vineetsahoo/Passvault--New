...
const SignUp: React.FC = () => {
  ...
  const [passwordStrength, setPasswordStrength] = useState<{ score: number; text: string; color: string }>({
    score: 0, text: 'Weak', color: '#CC0000',
  });
        ...
        <div className="w-full max-w-md">
          ...
          {errors.form && (
            <div className="border-2 border-[#CC0000] bg-[#FFE5E5] p-4 mb-8 flex items-start gap-3">
              ...
              <p className="text-sm text-[#CC0000]" style={{ fontFamily: "'Lora', serif" }}>{errors.form}</p>
            </div>
          )}
          ...
          <form onSubmit={handleSignUp} className="space-y-6">
            <div>
              <label className="block text-[0.65rem] uppercase tracking-[0.2em] mb-3 text-[#525252]" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                Full Name
              </label>
              <input type="text" name="name" value={formData.name} onChange={handleInputChange} className="input-newsprint" placeholder="John Doe" disabled={isLoading} />
            ...
            <div>
              ...
              <div className="relative">
                <input type="email" name="email" value={formData.email} onChange={handleInputChange} className="input-newsprint pr-8" placeholder="you@email.com" disabled={isLoading} />
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPassword ? 'text' : 'password'} name="password" value={formData.password} onChange={handleInputChange} className="input-newsprint pr-8" placeholder="••••••••" disabled={isLoading} />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-0 bottom-3 h-5 w-5 text-[#525252] hover:text-[#111111]">
              ...
              {formData.password && (
                <div className="flex gap-2 items-center text-xs mt-3">
                  <div className="flex-1 h-1.5 bg-[#E5E5E0] overflow-hidden">
                    <div className="h-full transition-all duration-300" style={{ width: `${(passwordStrength.score / 6) * 100}%`, backgroundColor: passwordStrength.color }} />
                  </div>
                  <span className="text-[0.65rem] uppercase tracking-widest font-bold" style={{ color: passwordStrength.color, fontFamily: "'JetBrains Mono', monospace" }}>
            ...
            <div>
              ...
              <div className="relative">
                <input type={showConfirmPassword ? 'text' : 'password'} name="confirmPassword" value={formData.confirmPassword} onChange={handleInputChange} className="input-newsprint pr-8" placeholder="••••••••" disabled={isLoading} />
                <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-0 bottom-3 h-5 w-5 text-[#525252] hover:text-[#111111]">
